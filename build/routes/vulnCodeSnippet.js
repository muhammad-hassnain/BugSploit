...
const retrieveCodeSnippet = async (challengeKey) => {
    ...
    if (codeChallenges.has(challengeKey)) {
        return (_a = codeChallenges.get(challengeKey)) !== null && _a !== void 0 ? _a : null;
    }
    return null;
};
...
exports.checkVulnLines = () => async (req, res, next) => {
    ...
    let hint;
    if (fs_1.default.existsSync('./data/static/codefixes/' + key + '.info.yml')) {
        const codingChallengeInfos = js_yaml_1.default.load(fs_1.default.readFileSync('./data/static/codefixes/' + key + '.info.yml', 'utf8'));
