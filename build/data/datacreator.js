    ...
    await Promise.all(challenges.map(async ({ name, category, description, difficulty, hint, hintUrl, mitigationUrl, key, disabledEnv, tutorial, tags }) => {
        ...
        try {
            datacache.challenges[key] = await challenge_1.ChallengeModel.create({
                ...
                description: effectiveDisabledEnv ? (description + ' <em>(This challenge is <strong>' + (config_1.default.get('challenges.safetyOverride') ? 'potentially harmful' : 'not available') + '</strong> on ' + effectiveDisabledEnv + '!)</em>') : description,
                difficulty,
                solved: false,
                hint: showHints ? hint : null,
                hintUrl: showHints ? hintUrl : null,
                mitigationUrl: showMitigations ? mitigationUrl : null,
                disabledEnv: config_1.default.get('challenges.safetyOverride') ? null : effectiveDisabledEnv,
...
async function createMemories() {
    const memories = [
        ...
        ...utils.thaw(config_1.default.get('memories')).map(async (memory) => {
...
async function createProducts() {
    const products = utils.thaw(config_1.default.get('products')).map((product) => {
        var _a, _b, _c;
